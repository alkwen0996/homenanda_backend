...
import com.home.nanda.exception.UnAuthorizedException;
...
public class JwtServiceImpl implements JwtService {
    ...
    private static final String SALT = "homeNandaSecret";
    private static final int ACCESS_TOKEN_EXPIRE_MINUTES = 1; // 분단위

    private static final int REFRESH_TOKEN_EXPIRE_MINUTES = 2; // 주단위

    @Override
    public <T> String createAccessToken(final String key, final T data) {
        return create(key, data, "access-token", 1000 * 60 * ACCESS_TOKEN_EXPIRE_MINUTES);
    }

    @Override
    public <T> String createRefreshToken(final String key, final T data) {
    ...
    @Override
    public <T> String create(final String key, final T data, final String subject, final long expire) {
        return Jwts.builder()
    ...
    public Map<String, Object> getKey(String key) {
        ...
        try {
            claims = Jwts.parser().setSigningKey(SALT.getBytes(UTF_8)).parseClaimsJws(jwt);
        } catch (Exception e) {
    ...
    public String getUserId() {
        return (String) this.getKey("user").get("userid");
    }
